   ...
   <div class="game-area">
    ...
    <div class="container">
        <transition-group name="rotate-all" appear="" class="card-container">
            <app-card
            :class="{'shadow' : selecetedCard == card.id}"
            @click.native="selecetedCard = card.id" 
            v-for="card in cards"
            :card="card" 
            :key="card.id">
            </app-card>
...
<script>
    ...
    export default {
        ...
        methods : {
            showCard(answer) {
                if(this.selecetedCard == null) {
                    alert("ilk olarak bir kart seçiniz!")
                }else {
                    this.activeCard = answer.component
                    setTimeout(() => {
                        if(answer.id == this.selecetedCard){
...
<style scoped>
    ...
    /******************** Açık kartların animasyonları için gerekli olan transition class tanimları *********************/
    ...
    /******************** Kapalı kartın animasyonları için gerekli olan transition class tanimları *********************/
    /* .rotate-enter{} */
    .rotate-enter-active{
